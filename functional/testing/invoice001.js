function ready(fn) {
  ...
  function updateInvoice(row) {
    try {
      ...
      if (!row.Subtotal && !row.Total && row.Items && Array.isArray(row.Items)) {
        try {
          row.Subtotal = row.Items.reduce((a, b) => a + b.Price * b.Cantitate, 0);
  ...
  ready(function() {
    ...
    grist.ready({   // This starts Grist
      requiredAccess: 'read table'    // Set access requirements
    });

    grist.onOptions(function(options, interaction) {    // Warning about requirements
